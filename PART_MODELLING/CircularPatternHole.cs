 public static int Startup()
{
	try
	{
		theProgram = new Program();
		theSession.ListingWindow.Open();
		string partPath = @"F:\CASE\TEST_PRT.prt";
		PartLoadStatus PLD;
		Part workPart = theSession.Parts.OpenBaseDisplay(partPath, out PLD) as Part;

		if (workPart != null)
		{
			theSession.ListingWindow.Open();
			Feature[] features = workPart.Features.ToArray();
			DisplayableObject[] datums =  workPart.Datums.ToArray();

			DatumPlane selectDatum = null;
			HolePackage hole = null;
			foreach(Feature feature in features)
			{
				if(feature.FeatureType == "HOLE PACKAGE")
				{
					hole = feature as HolePackage;
					break;
				}
			}
			foreach(DisplayableObject datum in datums)
			{
				if(datum.Name == "DATUM")
				{
					selectDatum = datum as DatumPlane;
					break;
				}
			}

			PatternFeature pattern = null;
			PatternFeatureBuilder builder = workPart.Features.CreatePatternFeatureBuilder(pattern);

			builder.FeatureList.Add(hole);
			builder.ReferencePointService.Point = workPart.Points.CreatePoint(hole.Location);
			builder.ParentFeatureInternal = false;

			Point3d origin = new Point3d(0, 0, 0);
			Vector3d axis = new Vector3d(0, 0, 1);
			Direction direct = workPart.Directions.CreateDirection(origin, axis, SmartObject.UpdateOption.DontUpdate);

			builder.PatternService.PatternType = NXOpen.GeometricUtilities.PatternDefinition.PatternEnum.Circular;
			builder.PatternService.CircularDefinition.RotationAxis = workPart.Axes.CreateAxis(new Point3d(100,200,0), axis, SmartObject.UpdateOption.DontUpdate);
			builder.PatternService.CircularDefinition.AngularSpacing.PitchAngle.RightHandSide = "180";
			builder.PatternService.CircularDefinition.AngularSpacing.NCopies.RightHandSide = "2";


			bool val = builder.Validate();
			builder.Commit();

		}

		PartSaveStatus ps;
		bool partSaved;
		theSession.Parts.SaveAll(out partSaved, out ps);

		theUFSession.Part.CloseAll();
	}
	catch (NXException nxe)
	{
		theSession.ListingWindow.Open();
		theSession.ListingWindow.WriteLine(nxe.ToString());
	}
	catch (Exception e)
	{
		theSession.ListingWindow.Open();
		theSession.ListingWindow.WriteLine(e.ToString());
	}

	return 0;
}